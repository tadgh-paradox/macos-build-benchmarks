...
JOBS="${JOBS:-}"
# Concurrent LTO link cap. Default 1 on this branch (`24`) — safe baseline for 24 GB hosts.
# `main` runs uncapped (matches production CI). Larger tiers: `32` (=2), `64` (=4). See SPEC §7.3.
...
Usage: $(basename "$0") [--target NAME] [--projects LIST] [--jobs N] [--no-clean]
  ...
  --projects LIST     Semicolon-separated LLVM subprojects. Default: 'clang;lld;mlir'.
                      ...
                      Env: LLVM_PROJECTS.
  --jobs N            Parallelism for cmake --build. Default on this branch (\`24\`):
                      nproc --ignore 2 emulation. Env: JOBS.
  --link-jobs N       Concurrent LTO link cap (-DLLVM_PARALLEL_LINK_JOBS). Default on this
                      branch (\`24\`): 1 — safe baseline for 24 GB hosts. \`main\` runs
                      uncapped (matches production CI). Larger tiers: \`32\` (=2), \`64\` (=4).
                      See SPEC §7.3. Env: LINK_JOBS.
