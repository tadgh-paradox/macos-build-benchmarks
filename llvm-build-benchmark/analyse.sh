...
MEMSTATS=""
BUILDLOG=""
LINK_TARGET="clang"   # LLVM's main user-facing binary target. Ninja prints "Linking CXX executable bin/clang-NN" — the regex picks up "clang" via the path-suffix match.
...
if [[ -n "$BUILDLOG" && -f "$BUILDLOG" ]]; then
  ...
  # cmake/ninja prints either bare target name or full path. Match: "executable <target>"
  # OR "executable /path/.../<target>" OR "executable /path/.../<target>-<version>" (LLVM names
  # its main binary `clang-22` rather than bare `clang`, so accept an optional -version suffix).
  link_start_line=$(grep -E "^\[[0-9:]+\].*Linking (CXX|C) executable .*[/ ]${LINK_TARGET}(-[0-9.]+)?([ /]|\$)" "$BUILDLOG" | head -1 || true)
